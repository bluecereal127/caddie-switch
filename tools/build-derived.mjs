...
import { flagCupMask } from "./lib/greensurface.mjs";
...
function transplantGreen(hole, img, g, box) {
  if (!box || !g?.sessions?.length) return;
  const t = greenTexture(g);
  if (!t) return;
  const W = img.width, H = img.height;
...
for (let hole = 1; hole <= 21; hole++) {
  ...
  // Only when the projection is confident. A weak correlation means the box
  // itself may be a few pixels off, and painting a whole green into the wrong
  // place is far worse than leaving the overview's own art alone (which is
  // already cup-free).
  if (box && (anchor?.r ?? 0) >= 0.45) transplantGreen(hole, img, g, box);
  else if (box) console.log(`H${hole}: green NOT transplanted — projection correlation only ${anchor?.r?.toFixed(2)}`);
